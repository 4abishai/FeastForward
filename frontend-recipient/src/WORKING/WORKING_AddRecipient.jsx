...
export default function AddRecipient() {
  ...
  const handleOpenHoursChange = (day, value) => {
    ...
    handleChange(`open_hours.${day}`, hours);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Filter out empty open hours
      const filteredOpenHours = {};
      Object.keys(formData.open_hours).forEach(day => {
        const hours = formData.open_hours[day].filter(h => h.trim() !== '');
        if (hours.length > 0) {
      ...
      if (response.status === 409) {
        const data = await response.json();
        alert(data.error || 'Recipient already exists');
        return;
      }
                    ...
                    {daysOfWeek.map(day => (
                      <div key={day} className="flex items-center space-x-2">
                        ...
                        <input
                          ...
                          value={formData.open_hours[day].join(', ')}
